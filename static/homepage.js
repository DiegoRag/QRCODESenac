...
function initializeApp() {
  ...
  const formLayout = [
    ...
    [
      ...
      {id: 'operador', label: 'Operador (Recebimento)', type: 'text', required: true}
    ],
    [
      {id: 'data-rec', label: 'Data de Emissão', type: 'date', required: true},
      {id: 'qtd-recebida', label: 'Qtd Recebida (Nota)', type: 'number', required: true}
    ],
    [
      {id: 'qtd-recebida-real', label: 'Qtd. Recebida Real', type: 'number', required: false},
      {id: 'qtd-amarrados', label: 'Qtd Amarrados (Auto)', type: 'number', required: false}
    ],
    [
    ...
    [
      {id: 'localizacao-ninho', label: 'Almoxarifado', type: 'text', required: false} 
    ],
    [{id: 'descricao-produto', label: 'Descrição do Produto', type: 'textarea', rows: 3, required: false}],
    [{id: 'observacao', label: 'Observações', type: 'textarea', rows: 3, required: false}],
    [
      {id: 'autorizado-por', label: 'Material Autorizado Por (Etapa 2)', type: 'text', required: false}
    ],
  ];
  ...
  formLayout.forEach(row => {
    ...
    row.forEach(field => {
      ...
      formHtml += `<div class="form-group" id="group-${field.id}" style="${isAuthField ? 'display: none;' : ''}">`;
      formHtml += `<label for="${field.id}">${field.label}</label>`;
      if (field.type === 'textarea') {
        formHtml += `<textarea id="${field.id}" rows="${field.rows || 3}" ${field.required ? 'required' : ''}></textarea>`;
      } else {
  ...
  // Adiciona o HTML do formulário ao DOM
  materialForm.innerHTML += formHtml;

  // Agora que o HTML do formulário existe, podemos buscar os elementos
  ...
  const calcularAmarrados = () => {
    const pesoBase = 500;
    // Certifica-se de que os elementos existem antes de ler o .value
    const qtdRealVal = qtdRecebidaRealInput ? parseFloat(qtdRecebidaRealInput.value) : 0;
    const qtdNotaVal = qtdRecebidaInput ? parseFloat(qtdRecebidaInput.value) : 0;
  ...
  if (qtdRecebidaInput) qtdRecebidaInput.addEventListener('input', calcularAmarrados);
  if (qtdRecebidaRealInput) qtdRecebidaRealInput.addEventListener('input', calcularAmarrados);
  ...
  // Confirm
  const showConfirmModal = (title, text, onConfirm, confirmText = 'Confirmar') => {
  ...
  function createQrItem(material, index) {
    ...
    let statusClass = 'status-pendente'; // Padrão
    ...
    deleteBtn.title = material.status === 'finalizado' ? 'Encerrar' : 'Excluir';
    ...
    // Todos os itens são clicáveis para ver detalhes
  ...
  showModalBtn.addEventListener('click', () => {
    ...
    document.getElementById('data-modal-title').textContent = 'Etapa 1: Criar Material';
    
    // *** CORREÇÃO 1: Adiciona verificação de null para o botão Salvar ***
    const saveBtn = document.querySelector('#material-form button[type="submit"]');
    if (saveBtn) saveBtn.style.display = 'block';
    
    // Re-bloqueia o campo de amarrados
    const qtdAmarradosInput = document.getElementById('qtd-amarrados');
    if (qtdAmarradosInput) {
        qtdAmarradosInput.readOnly = true;
        qtdAmarradosInput.style.backgroundColor = '#eef2ff';
  ...
  materialForm.addEventListener('submit', (event) => {
    ...
    const requiredIds = formLayout.flat()
      .filter(f => f.required)
      .map(f => f.id);
    ...
    if (autorizadoPor.trim() !== '' && almoxarifadoInput && !almoxarifadoInput.value.trim()) {
        showToast('O campo "Almoxarifado" é obrigatório para autorizar (Etapa 2).', 'error');
    ...
    if (existingId) {
      ...
      if (index !== -1) {
        ...
        if ((materialOriginal.status === 'pendente' || !materialOriginal.status) && autorizadoPor.trim() !== '') {
  ...
  const editMaterial = (id) => {
    ...
    materialForm.reset(); // Limpa o formulário antes de preencher
    ...
    formLayout.flat().forEach(field => {
        ...
        // *** CORREÇÃO APLICADA NA VERSÃO ANTERIOR ***
    ...
    // Reset all fields to not readonly first
    formLayout.flat().forEach(field => {
        const el = document.getElementById(field.id);
    ...
    if (material.status === 'encerrado') {
        ...
        if(saveBtn) saveBtn.style.display = 'none'; // Esconde o botão Salvar
        // Trava TODOS os campos
        formLayout.flat().forEach(field => {
            const el = document.getElementById(field.id);
        ...
        if(authInput) authInput.readOnly = true; // Trava só o campo de autorização
    } else {
        modalTitle.textContent = 'Etapa 2: Autorizar Material';
        if(authInput) authInput.readOnly = false; // Garante que está editável
    }
    
    // Sempre trava o campo de amarrados, pois é calculado
    const qtdAmarradosInput = document.getElementById('qtd-amarrados');
    if (qtdAmarradosInput) {
        qtdAmarradosInput.readOnly = true;
        qtdAmarradosInput.style.backgroundColor = '#eef2ff'; // Mantém o estilo de readonly
    }
  ...
  const deleteMaterial = (id, name) => {
    ...
    if (material.status === 'finalizado') {
      ...
      const title = material.status === 'encerrado' ? 'Excluir Permanentemente' : 'Excluir Material';
  ...
  const showMaterialDetails = (material) => {
    // Agora todos os status abrem o modal de detalhes
  ...
  pdfFileInput.addEventListener('change', async (event) => {
    ...
    // *** CORREÇÃO 2: Adiciona verificação de null para o statusEl.parentElement ***
  ...
  async function extractDataWithGemini(file, statusEl) {
    ...
    // ** CHAVE DE API ATUALIZADA CONFORME SOLICITADO **
...
Seja rigoroso e priorize os valores digitados, não os títulos exatos das seções.`;
    ...
    const schema = {
      type: "OBJECT",
      properties: {
        fornecedor: { type: "STRING", description: "Razão Social do EMITENTE da NF-e" },
        cnpjEntregador: { type: "STRING", description: "CNPJ do EMITENTE da NF-e" },
        denominacao: { type: "STRING", description: "Descrição principal do primeiro produto" },
        codPeca: { type: "STRING", description: "Código do primeiro produto" },
        item: { type: "STRING", description: "Número de volumes/pacotes da seção de transporte" },
        operador: { type: "STRING", description: "Sempre null, preenchido pelo usuário" },
        dataRec: { type: "STRING", description: "Data de Emissão (AAAA-MM-DD)" },
        qtdRecebida: { type: "NUMBER", description: "Peso Líquido Total da nota" },
        qtdRecebidaReal: { type: "NUMBER", description: "Sempre null, preenchido pelo usuário" },
        corrida: { type: "STRING", description: "Número da Corrida ou CHM (se houver)" },
        lote: { type: "STRING", description: "Número do Pedido do Cliente ou Lote" },
        localizacaoNinho: { type: "STRING", description: "Sempre null, preenchido pelo usuário" },
        descricaoProduto: { type: "STRING", description: "Descrição detalhada do primeiro produto" },
        observacao: { type: "STRING", description: "Sempre null, preenchido pelo usuário" }
      }
    };
